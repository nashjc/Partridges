...
% Author info
\title{Partridges in Pear Trees -- LaTeX}
\author{John Nash$^1$ \and Prashanth Velayudhan$^2$}

%% Why "date" function?

\date{
	$^1$Telfer School, University of Ottawa \\ \texttt{profjcnash@gmail.com}\\%
	$^2$Department of Physiology, University of Toronto \\ \texttt{psvelayu@gmail.com}\\[2ex]%
%	\today
}
...
The text preparation systems to be compared are LaTeX (\cite{lamport_latex_1986}), Typst (\cite{madje2022programmable}) and Rmarkdown (\cite{rmarkdown}). We note that bibliographic support is also of interest to users.
...
the Wikipedia reference \cite{noauthor_twelve_2025} as our authority.
...
progression (Equation~\ref{eq:aprog}).

\[
\label{eq:aprog} \tag{Arith-progrn} 
 S(k) = \sum _{i = 1} ^k {i} 
\]
...
\end{align*}
Table~\ref{tab:total} shows the total number of gifts received as of each day.
